...
\begin{table}[htbp]
    ...
    \begin{tabular}{lp{8cm}}
        ...
        \texttt{lwa\_f} & ZCU102 F-engine control library \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{llp{7cm}}
        ...
        \texttt{zcus}         & array of string & F-engine ZCU102 board hostnames \\
...
\subsection{\texttt{zcu} --- F-engine Configuration}

The \texttt{zcu} object configures the ZCU102 F-engine boards, including FPGA firmware, filter bank settings, and temperature thresholds (Table~\ref{tab:config-zcu}).

\begin{table}[htbp]
    \centering
    \caption{\texttt{zcu} Configuration Parameters}
    \label{tab:config-zcu}
    \begin{tabular}{llp{7cm}}
        \toprule
        \textbf{Key} & \textbf{Type} & \textbf{Description} \\
        \midrule
        \texttt{username}             & string         & SSH username for ZCU102 boards \\
        \texttt{password}             & string         & SSH password for ZCU102 boards \\
        \texttt{data\_ip\_base}       & string         & Base IP address for ZCU102 data interfaces \\
