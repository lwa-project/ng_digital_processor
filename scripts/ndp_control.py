...
"""
NDP main control service
To be run on NDP headnode
...
__copyright__  = "Copyright 2023, The Long Wavlength Array Project"
...
def main(argv):
	parser = argparse.ArgumentParser(description='LWA-NA NDP control service')
	...
	try:
		services = {
			'msg_receiver':  MCS2.MsgReceiver(recv_addr, subsystem=Ndp.SUBSYSTEM),
			'msg_processor':  Ndp.MsgProcessor(config, log, dry_run=args.dryrun),
			'msg_sender':    MCS2.MsgSender(  send_addr, subsystem=Ndp.SUBSYSTEM)
		}
	except MCS2.socket.error as e:
		if e.errno == 98:
			log.error('Socket address in use; '
			          'is ndp-control already running?')
			sys.exit(-e.errno)
