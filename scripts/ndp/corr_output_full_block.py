...
class CorrOutputFull(Block):
    ...
    **Functionality**
    ...
    Output a visibility buffer as a stream of UDP packets.
    ...
    **Input Header Requirements**
    ...
    Optional header fields, which describe the input TCC buffer contents. If not
    ...
    *Input Data Buffer*: A CPU-side bifrost ring buffer of 32+32 bit complex integer data.
    This input buffer is read in gulps of ``nchan * nstand*(nstand+1)//2*npol*npol*2``
    32-bit words, which is the size of an TCC visibility matrix.
