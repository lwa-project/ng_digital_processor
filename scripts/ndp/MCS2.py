...
# Simple interface for communicating with ndp-control service
class Communicator(object):
    def __init__(self, subsystem='MCS'):
        ...
        self.sender   = MsgSender(("ndp",1742), subsystem=subsystem)
    ...
    def report(self, data, fmt=None, timeout=5.):
        msg = Msg(dst='NDP', cmd='RPT', data=data)
    ...
    def command(self, cmd, data='', timeout=5.):
        msg = Msg(dst='NDP', cmd=cmd, data=data)
...
class Synchronizer(object):
    def __init__(self, group, addr=("ndp",23820)):
        self.group  = group
        self.addr   = addr
