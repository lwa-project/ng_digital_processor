...
class NdpFileHandler(TimedRotatingFileHandler):
    ...
    def doRollover(self):
        super(NdpFileHandler, self).doRollover()
        if self.rollover_callback is not None:
            self.rollover_callback()

class NdpFileLogger(object):
    def __init__(self, config, filename, fileheader, *args, **kwargs):
        ...
        self._handler = NdpFileHandler(config, filename,
